  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/auth/signup', {
        ...
        role: 'SALES' // Defaulting to SALES as per UI text
      });
    ...
    <div className="flex min-h-screen bg-white">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 lg:p-24 bg-white">
        <div className="w-full max-w-lg">
          ...
          <h3 className="text-xs font-mono tracking-widest text-brand-light mb-4 uppercase">New Account</h3>
          <h2 className="text-4xl sm:text-5xl font-serif text-brand-dark mb-6 tracking-tight">Create account</h2>
          <p className="text-slate-500 mb-12 text-base leading-relaxed">
            Every new account opens on the Sales desk. An admin can move you later.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              <div className="relative group">
                <label className="absolute -top-6 left-0 text-xs font-mono tracking-widest text-brand-light uppercase flex items-center gap-2">
                  <span className="text-[10px]">01</span> NAME
                </label>
                <input
                  type="text"
                  name="name"
              ...
              <div className="relative group">
                <label className="absolute -top-6 left-0 text-xs font-mono tracking-widest text-brand-light uppercase flex items-center gap-2">
                  <span className="text-[10px]">02</span> EMAIL
                </label>
                <input
                  type="email"
                  name="email"
                  ...
                  onChange={handleChange}
                  placeholder="rahul@fundsroom.com"
                  required
                  className="w-full bg-transparent border-b border-slate-200 py-2 text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-brand-dark transition-colors"
                />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mt-8">
              ...
              <div className="relative group">
                <label className="absolute -top-6 left-0 text-xs font-mono tracking-widest text-brand-light uppercase flex items-center gap-2">
                  <span className="text-[10px]">03</span> PASSWORD
                </label>
                <input
                  type="password"
                  name="password"
              ...
              <div className="relative group">
                <label className="absolute -top-6 left-0 text-xs font-mono tracking-widest text-brand-light uppercase flex items-center gap-2">
                  <span className="text-[10px]">04</span> CONFIRM PASSWORD
                </label>
                <input
                  type="password"
            ...
            {success && (
              <div className="text-green-600 text-sm mt-4 font-medium">{success}</div>
            )}

            <button
              type="submit"
              disabled={loading}
              className="w-full bg-brand-dark hover:bg-slate-800 text-white font-mono text-sm tracking-[0.2em] py-4 mt-8 transition-colors disabled:opacity-70 disabled:cursor-not-allowed"
            >
          ...
          <p className="mt-8 text-slate-500 text-sm">
            Already have an account? <a href="/login" className="text-brand-light hover:text-brand-dark underline decoration-1 underline-offset-4 transition-colors">Sign in</a>
          </p>
